...
export const GET = withAuth(async (_req, { params, user }) => {
  ...
  await connectDB();
  const doc = await Freelance.findOne({ _id: id, username: user.username }).lean();
...
export const PATCH = withAuth(async (req, { params, user }) => {
  ...
  delete body.username; delete body._id; delete body.id;
...
export const DELETE = withAuth(async (_req, { params, user }) => {
  const { id } = await params;
  await connectDB();
  const deleted = await Freelance.findOneAndDelete({ _id: id, username: user.username });
