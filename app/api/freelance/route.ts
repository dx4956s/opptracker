...
export const GET = withAuth(async (req, { user }) => {
  ...
  const allowedSort = ["createdAt", "updatedAt", "client", "bidDate", "startDate"];
  const sortField = allowedSort.includes(sort) ? sort : "createdAt";

  const total = await Freelance.countDocuments(query);
  const docs  = await Freelance.find(query)
...
export const POST = withAuth(async (req, { user }) => {
  ...
  await connectDB();
  const doc = await Freelance.create({ ...body, username: user.username });
